-- Ensure email column, readable policies, reliable lead upsert RPC, and backfill
-- from existing waitlist_signup / phone_captured events.
...
-- Prototype admin reads (tighten before production).
...
-- Upsert that works even when PostgREST upsert + RLS is picky.
...
-- Recover signups already captured as events but never written to leads.
INSERT INTO public.leads (visitor_id, phone, email, prefs, user_agent, created_at, updated_at)
